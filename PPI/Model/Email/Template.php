 ...
 * @package   Model
 */
namespace PPI\Model\Email;

use PPI\Core\CoreException;

class Template extends \PPI\Model {
    private $_sender;
    private $_recipient;
    private $_body;
    private $_subject;
    ...
    function setTemplate($p_sTemplate, $p_aReplacerData = array()) {
        ...
        if(count($rows) < 1) {
            throw new CoreException('Trying to use Email Template: '.$p_sTemplate . ' but it doesn\'t exist');
        }
    ...
    function sendMail() {
        ...
        if(is_array($this->_recipient) && count($this->_recipient) < 1) {
            throw new CoreException('Unable to send email: No recipient specified');
        } elseif(is_string($this->_recipient) && $this->_recipient == '') {
            throw new CoreException('Unable to send email: No recipient specified');
        }
        if($this->_sender == '') {
            throw new CoreException('Unable to send email: No sender specified');
        }
        if($this->_subject == '') {
            throw new CoreException('Unable to send email: No subject specified');
        }
